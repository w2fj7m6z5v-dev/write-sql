...
| 入网时设备机身码 / 入网时光猫 / 订单设备 | `subs_id`（069入网订单标识） | 需要取入网时绑定的设备，而非当前设备 | 144 CRM设备订单关系表 `dws_crm_order.dws_ord_prod_res_inst_rel/_his` | **关联键是 `order_item_id`（=069`subs_id`），不是 `prod_inst_id`**；归档表`_his` + 当前表 UNION ALL，`status_cd='1100'`，`city_id=200`；输出 `eqpt_sn`（机身码） | **区别于119**：119按`serv_id`取号码当前设备，本表按`order_item_id`取入网订单绑定的设备；同一订单可能多设备（光猫+机顶盒），如只需光猫需加筛选 |
