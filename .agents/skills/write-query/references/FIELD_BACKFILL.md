...
| 装机地址标识 / 标准地址 ID | 主表 `serv_id + par_month_id` | 主表缺 `serv_addr_id`，但需求要标准装机地址标识、标准装机地址或后续地址层级 | 069 全业务资料表 `dwm_yz_tb_comm_cm_all_final` / `view_ads_yz_tb_comm_cm_all_final` | `主表.serv_id = 069.serv_id AND 主表.par_month_id = 069.par_month_id` → `069.serv_addr_id`；若还要中文地址或层级，再按 `serv_addr_id` 补 079 地址维表 | 不要在主表文档未列字段时硬写 `主表.serv_addr_id`；不要用 128 的 `address_desc` 替代标准地址标识；同一服务同账期异常多行时需先按 `serv_id,par_month_id` 去重 |
...
| 入网时设备机身码 / 入网时光猫 / 订单设备 | `subs_id`（069入网订单标识） | 需要取入网时绑定的设备，而非当前设备 | 140 CRM设备订单关系表 `dws_crm_order.dws_ord_prod_res_inst_rel/_his` | **关联键是 `order_item_id`（=069`subs_id`），不是 `prod_inst_id`**；归档表`_his` + 当前表 UNION ALL，`status_cd='1100'`，`city_id=200`；输出 `eqpt_sn`（机身码） | **区别于119**：119按`serv_id`取号码当前设备，本表按`order_item_id`取入网订单绑定的设备；同一订单可能多设备（光猫+机顶盒），如只需光猫需加筛选 |
...
| 非费用账目使用记录 / 全产品使用记录 | `serv_id`、`billing_cycle_id` | 需要判断某客户/某服务在指定月份是否有实际使用行为（时长类非费用项），用于欠费追缴、使用记录查询 | 143 非费用账目表 `dws_acct.dws_unacct_item` | `主表.serv_id = 143.prod_inst_id`；按 `billing_cycle_id` 对齐账期；`city_id='200'`；`acct_item_type_id` 列表按省公司指引传入 | 与 116 固话使用记录表（仅固话通话时长）不同；`acct_item_type_id` 码值列表不硬编码；同一 `prod_inst_id` 同一 `billing_cycle_id` 可能多科目，需 `group by billing_cycle_id, prod_inst_id` 聚合 `sum(amount)` |
...
| **附属产品订单历史 / 附属产品开通取消记录** | `serv_id` | 查附属产品（如全网通）的开通/取消时间、订单流水 | **141 附属产品订单表族**（`tables/141_附属产品订单表族.md`） | UNION ALL 四表（日/月 × 当前/历史）按 `serv_id` + `sub_prod_id` 过滤；`oper_type='1000'`=开通，`'1100'`=取消；`subs_stat='301200'`=竣工；取最新 `subs_stat_date` | 不是 106 属性快照；查历史必须 UNION ALL 含 hist 表；按 `subs_stat_date desc` 取最新一条 |
