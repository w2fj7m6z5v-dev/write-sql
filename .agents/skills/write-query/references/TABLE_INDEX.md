...
| 141 | 附属产品订单表族 | summary_ods_day_city.rpt_comm_ba_sub_prod | summary_ods_day_city.rpt_comm_ba_sub_prod（日-当前）；summary_ods_day_city.rpt_comm_ba_sub_prod_hist（日-历史）；iodata_ods_month_city.rpt_comm_ba_sub_prod_mon（月-当前）；iodata_ods_month_city.rpt_comm_ba_sub_prod_hist_mon（月-历史） | tables/141_附属产品订单表族.md | 附属产品订单流水；serv_id + sub_prod_id + subs_code 粒度 | par_corp_id, par_month_id（月表） | 附属产品开通/取消订单流水；按 oper_type(1000=开通/1100=取消)+subs_stat(301200=竣工) 过滤 | 与 106 属性快照不同；查历史订单需 UNION ALL 四表 |
| 142 | 专线月租表 | ads_yz_sx_yz | ads_yz_sx_yz | tables/142_专线月租表.md | 服务-月份粒度 | par_month_id | 专线月租查询；按 `serv_id + par_month_id` 取 `yz_cs_old` | 与 033 yz_cs 双线口径不同；本表专线旧口径 |
| 143 | 非费用账目表 | dws_acct.dws_unacct_item | dws_acct.dws_unacct_item | tables/143_非费用账目表.md | 账期-产品实例-科目明细粒度 | 待确认 | 全产品非费用账目使用记录查询；按 `acct_item_type_id` 过滤时长类非费用项判断使用行为 | 与 116 固话使用记录表不同；`acct_item_type_id` 列表不硬编码 |
