...
| 143 | 未出账明细表 | dws_acct.dws_unacct_item | dws_acct.dws_unacct_item | tables/143_未出账明细表.md | 账期-产品实例-科目明细粒度 | 待确认 | 全产品未出账使用记录查询；按 `acct_item_type_id` 过滤时长类非费用项判断使用行为 | 与 116 固话使用记录表不同；`acct_item_type_id` 列表不硬编码 |
