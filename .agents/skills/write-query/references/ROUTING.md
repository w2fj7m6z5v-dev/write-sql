...
| 入网时设备机身码 / 入网时光猫 / 订单设备 | 入网订单维度的设备信息 | 069 取 `subs_id` → 144 CRM设备订单关系表 | `order_item_id`（=069 `subs_id`）；输出 `eqpt_sn`；归档表_his + 当前表 UNION ALL；`status_cd='1100'`；`city_id=200` | **区别于119号码设备表**：119按`serv_id`取当前设备，本表按`order_item_id`取入网时设备；需要"入网时的设备"必须走本表；不要用`prod_inst_id`做主关联键 |
...
| 入网时设备机身码 / 入网时光猫 | 用户附件种子表 → 069 取 `subs_id` | 144 CRM设备订单关系表 `dws_crm_order.dws_ord_prod_res_inst_rel/_his` | 119 号码设备关系表（取的是当前设备，不是入网时设备） | **关键区分**：119按`serv_id`取号码当前设备，本表按`order_item_id`(=069`subs_id`)取入网订单绑定的设备；归档表_his+当前表UNION ALL；`status_cd='1100'`；`city_id=200` |
